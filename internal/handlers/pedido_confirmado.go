...
func CreatePedidoConfirmado(w http.ResponseWriter, r *http.Request) {
	// Parse multipart form
	err := r.ParseMultipartForm(100 << 20) // 100 MB
	if err != nil {
		http.Error(w, "Invalid multipart form", http.StatusBadRequest)
	...
	templatePayload := fmt.Sprintf(`{
		...
		"components": [
			...
			{
				"type": "body",
				"text": "Hola, {{1}}:\n\n¡Gracias por tu compra en *{{2}}*\n\nTe compartimos los detalles:\nFolio es *{{3}}*.\nCantidad de productos *{{4}}*\nMonto total: *{{5}}*\nVendedor *{{6}}*\n\nEstamos trabajando para tener listo tu envío en la fecha estipulada.",
				"example": {
					"body_text": [["Alex Velasco", "Muebleria S", "PAX001", "4", "10,000", "Juan Perez"]]
				}
			}
		]
	}`, uploadFileResult.H)
