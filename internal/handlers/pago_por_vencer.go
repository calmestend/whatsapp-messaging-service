...
// Struct with parameters for template "pago_por_vencer" message
type PagoPorVencerRequest struct {
	models.MessageRequest
	Sale   string `json:"sale" validate:"required"`
	Amount string `json:"amount" validate:"required"`
	Days   string `json:"days" validate:"required"`
}

// Handle template "pago_por_vencer" from the Whatsapp Business API from Meta
func PagoPorVencer(w http.ResponseWriter, r *http.Request) {
	// Parse json
	var payloadData PagoPorVencerRequest
...
}`, payloadData.To, payloadData.Sale, payloadData.Amount, payloadData.Days)
...
func CreatePagoPorVencer(w http.ResponseWriter, r *http.Request) {
	// Parse form
		...
		"components": [
			{
				"type": "header",
				"format": "text",
				"text": "Pago por Vencer"
			},
			{
				"type": "body",
				"text": "El pago de tu venta {{1}} por {{2}} vence en {{3}} días.\n\nPaga a tiempo y evita cobro de intereses.\nPonte en contacto con nosotros si necesitas ayuda.",
				"example": {
					"body_text": [["folio", "$12,34", "3"]]
				}
			}
		]
	}`
