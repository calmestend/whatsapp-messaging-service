...
// Struct with parameters for template "saldo_vencido" message
type SaldoVencidoRequest struct {
...
// Handle template "saldo_vencido" from the Whatsapp Business API from Meta
func SaldoVencido(w http.ResponseWriter, r *http.Request) {
	// Parse json
	var payloadData SaldoVencidoRequest
...
}`, payloadData.To, payloadData.Account, payloadData.Amount, payloadData.Days)
...
func CreateSaldoVencido(w http.ResponseWriter, r *http.Request) {
	// Parse form
		...
		"components": [
			{
				"type": "header",
				"format": "text",
				"text": "Saldo Vencido en tu crédito"
			},
			{
				"type": "body",
				"text": "Actualmente tienes un saldo total vencido en tu cuenta {{1}} por {{2}} que venció hace {{3}} días.\n\nFavor de pasar a liquidar su deuda. Ponte en contacto con nosotros si necesitas ayuda.",
				"example": {
					"body_text": [["2M2287", "$10,000", "15"]]
				}
			}
		]
	}`
